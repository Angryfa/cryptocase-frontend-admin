...
export default function CaseCreatePage() {
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      const resTypes = await authFetch("/api/cases/types/");
      const dataTypes = await resTypes.json();
      if (mounted) setTypes(dataTypes);
    })();
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const res = await authFetch("/api/admin/cases/", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      ...
      <div className={`${s.card} ${s.cardNarrow}`}>
        <form onSubmit={onSubmit} className={s.formCompact}>
          <div className={s.cols2}>
            <div className={s.field}><label>Название</label><input value={name} onChange={e=>setName(e.target.value)} required /></div>
            <div className={s.field}><label>Цена, $</label><input type="number" value={price} onChange={e=>setPrice(e.target.value)} required /></div>
